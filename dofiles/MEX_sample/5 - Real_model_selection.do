...
*===============================================================================
	...
	*foreach v of global myvar{
	*	sum `v'
	*	if r(mean) == 0 global myvar: list myvar - `v'
	*}
	...
	*lasso linear bcy $myvar [iw=Whh], selection(cv, serule folds(10))
	local hhvars = e(varlist_nonzero)
	global postlasso = e(varlist_nonzero)
	
	//Try Henderson III GLS
	sae model h3 bcy $postlasso [aw=Whh], area(HID_mun) 
		...
		foreach x of varlist `hhvars'{
			...
			if (r(p)>`z'){
				...
				foreach yy of local hhvars{
					if ("`yy'"=="`x'") noi dis "not-significance: `x'"
...
/*
This code performs a stepwise backward elimination using the Henderson III GLS small-area model:
1. Start with all candidate variables.
2. Test significance at a given α.
3. Drop variables with p > α.
4. Tighten α (make it smaller) and repeat.
5. Stop when every coefficient left in the model is statistically significant.
Essentially, it's an automated process to simplify the model by iteratively removing weak predictors until only strong, statistically meaningful ones remain.

The loop gradually tightens the significance threshold (α = 0.5 → 0.05) to systematically and stably remove weak predictors, ensuring the final SAE model contains only variables that remain statistically robust across increasingly strict criteria.

"H3" tells the SAE command to fit a unit-level mixed model using Henderson's Method III GLS,
which jointly estimates fixed-effect coefficients and area-specific random effects by efficiently weighting household-level and area-level variation.
*/
	...
	*mata: ds = _f_stepvif("`hhvars'","Whh",5,"touse")  //is used to remove covariates with a VIF above a specified threshold, it comes with SAE pakage
...
*===============================================================================	
	...
	*loc allvars "$myvar"
	...
	sae model h3 bcy $postvif if nogo==0 [aw=Whh], area(HID_mun) ///
...
*===============================================================================
	...
	forval z= 0.5(-0.05)0.05{
		qui:sae model h3 bcy `hhvars' if nogo==0 [aw=Whh], area(HID_mun) ///
		zvar($alfavars)
		mata: bb=st_matrix("e(b_gls)")
		mata: se=sqrt(diagonal(st_matrix("e(V_gls)")))
		mata: zvals = bb':/se
...
*===============================================================================
sae model h3 bcy $postalfa if nogo==0 [aw=Whh], area(HID_mun) zvar($alfavars)

char _dta[beta]   $postalfa

...
cd "C:\Users\wb545671\OneDrive - WBG\AFE_work\Mission\5.UGA_Nov2025\UGA_training\01.dofiles\Day1"

*===============================================================================
//1. Let's create a population. Just like we did before in dofiles 1 and 2
*===============================================================================
global numpop    = 10 //Number of populations to generate 
...
glo sigmaeta = 0.15     //Sigma eta (random area effect)
glo sigmaeps = 0.5      //Sigma eps (hh ind error)
...
save dpopulation, replace //note that we haven't added errors here...those are random 
...
estimation of MSEs under the parametric  .
...
use dpopulation, clear
	// Generate the 10 true welfare vectors...
	forval np=1/$numpop{
...
gen povline3 = ${pline50}

forvalues i = 1/10{
	gen povrate`i' = e_y_`i' < povline3
}

collapse (mean) e_y_* povrate*, by(area)

reshape long povrate e_y_, i(area) j(sim)
...
forval np=1/$numpop{
	...
	aggids(0) ind(fgt0 fgt1 fgt2) uniqid(hhid) area(area) //appendsvy refer to EB
...
forval np=1/$numpop{
	...
	matin(mfile) pwcensus(hhsize) plines($pline10 $pline25 $pline50 $pline75) ///
	aggids(0) ind(fgt0 fgt1 fgt2) uniqid(hhid) area(area) eta(normal) epsilon(normal) allmata
...
use true, clear
	...
	foreach method in EB CEB h3CEB ELL{
...
collapse (mean)bias_* mse_*, by(area)
